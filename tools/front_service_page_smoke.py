#!/usr/bin/env python3
"""Smoke-check the front customer-service page in a real browser."""
...
def main() -> int:
    ...
    parser.add_argument("--base-url", default="http://127.0.0.1:18080/public/service.php")
    ...
    with sync_playwright() as playwright:
        ...
        for region in ["macau", "hongkong"]:
            for width in widths:
                ...
                check = page.evaluate(
                    """({region}) => {
                      const servicePage = document.querySelector('.customer-service-page');
                      const frame = document.querySelector('.customer-service-frame');
                      const thread = document.querySelector('.service-thread--member, .service-thread--agent');
                      ...
                      return {
                        ...
                        hasServicePage: !!servicePage,
                        hasFrame: !!frame,
                        hasThread: !!thread,
                        threadDisplay: threadStyle ? threadStyle.display : '',
